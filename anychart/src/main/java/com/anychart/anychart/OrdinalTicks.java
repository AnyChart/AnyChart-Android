...
public class OrdinalTicks extends CoreBase {
    ...
    public void setNames(String[] values) {
        if (jsBase == null) {
            ...
            js.append(String.format(Locale.US, jsBase + ".names(%s);", Arrays.toString(values)));

            if (isRendered) {
                onChangeListener.onChange(String.format(Locale.US, jsBase + ".names(%s);", Arrays.toString(values)));
                js.setLength(0);
            }
        }
    }
